...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def _prepare_inputs(
        ...
        if np.any(num_kv_cache_tokens < 0):
            debug_info = {
                'req_ids': self.input_batch.req_ids[:num_reqs],
                'num_computed': self.input_batch.num_computed_tokens_cpu[:num_reqs].tolist(),
                'num_dropped': self.input_batch.num_dropped_tokens_list_cpu[:num_reqs].tolist(),
                'num_scheduled_tokens': num_scheduled_tokens.tolist(),
            }
            raise RuntimeError(f'Negative KV cache tokens detected: {debug_info}')
